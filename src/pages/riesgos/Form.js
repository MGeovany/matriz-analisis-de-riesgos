...
export default function Form({
  ...
  React.useState(() => {
    ...
    Opciones()
      .Posibilidad()
      .then((data) => setListaPosibilidad(data))
      .catch((error) => console.error(error))
    Opciones()
      .NivelRiesgo()
  ...
  function submitForm() {
    handler({
      ...form.values,
      PuntajeImpacto: ListaImpacto.find(
        (item) => item.value === form.values.IdImpacto
      ).puntaje,
      NivelImpacto: ListaImpacto.find(
        (item) => item.value === form.values.IdImpacto
      ).label,
      PuntajePosibilidad: ListaPosibilidad.find(
            ...
            <Grid.Col md={4} xs={12}>
              <Stack>
                ...
                <span>
                  ...
                  <NivelRiesgoItem
                    puntaje={getPuntaje(
                      form.values.IdImpacto,
                      form.values.IdPosibilidad
                    )}
                    texto={getNivelRiesgo(
                      getPuntaje(
                        form.values.IdImpacto,
                        form.values.IdPosibilidad
                      )
              ...
              <Stack>
                ...
                <Group>
                  <Checkbox
                    label='Afecta Costo'
                    radius='md'
                    {...form.getInputProps('AfectaCosto', { type: 'checkbox' })}
                  />
                  {form.getInputProps('AfectaCosto').value ? (
                    <TextInput
                      placeholder='$.'
                      size='xs'
                      {...form.getInputProps('ValorCosto')}
                    />
                  ) : null}
                </Group>
                <Group>
                  <Checkbox
                    label='Afecta Tiempo'
                    radius='md'
                    {...form.getInputProps('AfectaTiempo', {
                      type: 'checkbox'
                    })}
                  />
                  {form.getInputProps('AfectaTiempo').value ? (
                    <TextInput
                      size='xs'
                      placeholder='Horas'
                      {...form.getInputProps('ValorTiempo')}
                    />
                  ) : null}
                </Group>
                ...
                <Group>
                  <Checkbox
                    label='Afecta Calidad'
                    radius='md'
                    {...form.getInputProps('AfectaCalidad', {
                      type: 'checkbox'
                    })}
                  />
                </Group>
  ...
  function getPuntaje(IdImpacto, IdPosibilidad) {
    const impacto = ListaImpacto.find((item) => item.value === IdImpacto) || {
      puntaje: 0
    }
    const posibilidad = ListaPosibilidad.find(
      (item) => item.value === IdPosibilidad
    ) || { puntaje: 0 }
